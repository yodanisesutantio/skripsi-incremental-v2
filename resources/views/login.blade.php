...
@section('content')
    ...
    <div class="flex flex-col gap-1">
        <h1 class="text-3xl lg:text-4xl/snug tracking-tight text-custom-dark font-encode font-semibold">Login</h1>
        <p class="text-custom-grey text-lg font-league lg:text-xl">Belum punya akun? <a href="/register" class="text-custom-green font-semibold hover:underline">Daftar Sekarang</a></p>
    </div>

    {{-- Forms --}}
    <form action="/login" method="post">
        @csrf
        <div class="flex flex-col mt-8 lg:my-12 gap-5 lg:gap-7">
            <div class="flex flex-col gap-1">
                ...
                <input type="text" name="username" id="username" autofocus placeholder="Username" class="p-4 font-league text-lg/[0] text-custom-secondary placeholder:#48484833 rounded-lg @error('username') border-2 border-custom-destructive @enderror" value="{{ old('username') }}">
                @error('username')
